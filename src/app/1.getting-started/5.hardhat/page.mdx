...
code editor like **VSCode**. For better performance, we use
[bun](https://bun.sh) as our package manager.
...
<Tutorial>
  <Steps>
    ...
    <Step outputId="scripts">
      `scripts`: A set of scripts that can be run using `bun run {script-name}`
    </Step>

    <Step>
      In your termainal, run the following command to install Hardhat <br />
      ```bash
      bun add @nomicfoundation/hardhat-toolbox hardhat @nomicfoundation/hardhat-chai-matchers @nomicfoundation/hardhat-ethers @nomicfoundation/hardhat-ignition @openzeppelin/contracts dayjs dotenv ethers solidity-coverage chai@4.4.1 @types/chai
      ```
    </Step>
